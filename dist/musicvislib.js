// https://fheyen.github.io/musicvis-lib v0.46.4
(function (global, factory) {
    ...
    class NoteArray {
         ...
         */
        transpose(steps) {
            this._notes = this._notes.map(n => Note$1.from({
                ...n,
         ...
         */
        removeOctaves() {
            this._notes = this._notes.map(note => Note$1.from({
                ...note,
                pitch: note.pitch % 12,
            }));
            return this;
    ...
    */
    ...
    function preprocessMusicXmlPart(part, drumInstrumentMap) {
        ...
        for (const measure of measures) {
            const currentTimeRounded = roundToNDecimals(currentTime, ROUNDING_PRECISION$1);
                ...
                if (child.nodeName === 'backup') {
                    ...
                    try {
                        ...
                        if (tieElement && tieElement.getAttribute('type') === 'stop') {
                            ...
                            const endTime = roundToNDecimals(currentTime + durationInSeconds, ROUNDING_PRECISION$1);
    ...
    class MusicPiece {
        ...
        static fromMidi2(name, midiFile) {
            ...
            const parsed = new Midi_2(midiFile);
     ...
     */
    const drumPitchReplacementMapMPS850 = new Map([
        // Crash 1
    ...
    function simplifyDrumPitches(notes, replacementMap) {
        ...
        const simplified = notes.map(note => {
            ...
            const newNote = Note$1.from({ ...note, pitch: newPitch });
            return newNote;
        });
     ...
     */
    const lamellophoneTunings = new Map([
        ['Kalimba', new Map([
            [
                '9 A Major',
     ...
     */
    function convertNotesToHtmlTab(
        notes,
        tuning,
        mode = 'letter',
        restSize = 0.1,
    ...
    function testAlignment() {
        ...
        const a = new NoteArray([
            new Note$1(69, 0, 127, 0, 1),
            new Note$1(70, 1, 127, 0, 2),
            new Note$1(71, 2, 127, 0, 3),
        ]);
        ...
        b = a.clone()
            .shiftTime(3)
            .addNotes([new Note$1(72, 2, 127, 0, 3)]);
    ...
    function alignmentBenchmark() {
        ...
        const randDuration = randomUniform.source(lcg(seed))(1 / 64, 2);

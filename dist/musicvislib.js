var musicvislib = (() => {
  ...
  function normalizeNdArrayNegative(array) {
    const extent = extent_default(array.flat(Number.POSITIVE_INFINITY));
    const scale2 = linear2().domain(extent);
    const normalize2 = (array_) => array_.map((d) => {
      return d.length !== void 0 ? normalize2(d) : scale2(d);
    });
    return normalize2(array);
  }
        ...
        if (child.nodeName === "backup") {
          ...
          try {
            ...
            if (isUnpitched) {
              ...
              const alter = +(((_a = note2.querySelectorAll("alter")[0]) == null ? void 0 : _a.innerHTML) ?? 0);
            ...
            if (tieElement && tieElement.getAttribute("type") === "stop") {
              ...
              const staff = +(((_c = note2.querySelectorAll("staff")[0]) == null ? void 0 : _c.innerHTML) ?? 1);
  ...
  function preprocessMidiFileData(data, splitFormat0IntoTracks = true, log = false) {
    ...
    const totalTime = max(parsedTracks, (d) => (d == null ? void 0 : d.totalTime) ?? 0);
  ...
  var Track = class {
    ...
    getMeasures(sortComparator) {
      ...
      const measures = [];
      for (let index16 = 1; index16 < indices.length; ++index16) {
        const notes = this.notes.slice(indices[index16 - 1], indices[index16]);
        if (sortComparator) {
    ...
    getSectionInfo() {
      ...
      for (let index16 = 1; index16 < sections.length; ++index16) {
        sections[index16 - 1].endMeasure = sections[index16].startMeasure - 1;
      }
    ...
    getSections(sectionInfo, measures, sortComparator) {
      ...
      for (let index16 = 1; index16 < indices.length + 1; ++index16) {
        const notes = measures.slice(indices[index16 - 1], indices[index16]).flat();
        if (sortComparator) {
          notes.sort(sortComparator);
        }
    ...
    for (const chord2 of chords2) {
      let chordString = chord2.map((note2) => {
        ...
        if (pitchToSymbolMap.has(note2.pitch)) {
          ...
          return mode2 === "letter" ? ((_a = getMidiNoteByNr(note2.pitch)) == null ? void 0 : _a.name) ?? note2.pitch : note2.pitch;
        }
      }).join(" ");
      if (chord2.length > 1) {
    ...
    for (const chord2 of chords2) {
      let chordString = chord2.map((note2) => {
        ...
        if (pitchToSymbolMap.has(note2.pitch)) {
          ...
          string = mode2 === "letter" ? ((_a = getMidiNoteByNr(note2.pitch)) == null ? void 0 : _a.name) ?? note2.pitch : note2.pitch;
        }
